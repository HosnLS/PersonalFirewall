...
NTSTATUS SioctlDeviceControl(
	...
	case IOCTL_SIOCTL_METHOD_BUFFERED:
		...
		retLength = UProcessIO((PCHAR)(Irp->AssociatedIrp.SystemBuffer), inBufLength, outBufLength);
		Irp->IoStatus.Information = outBufLength < retLength ? outBufLength : retLength;
		break;
...
LONG CheckNetBufferList(
	...
	LONG DataTotalLength = 0, DataCurrentLength = 0, DataMdlLength = 0, lOffset = 0, rOffset = 0, ujudgeResult = 1;
	...
	do {
		...
		// move memory
		if (pDataMdlAddr + lOffset > 0) NdisMoveMemory(pDataCopyAddr, pDataMdlAddr + lOffset, rOffset);
...
End:
	*/
	...
	ujudgeResult = UJudgeFrame(pDataCopyAddr, DataTotalLength, Mode);
...
--*/
{
	...
	do {
		...
		if (pFilter->TrackSends) {
			...
			Pass = CheckNetBufferList(pFilter, NetBufferLists, U_CNBL_SEND);
			if (Pass)
...
--*/
{
	...
	do {
		...
		if (pFilter->TrackReceives) {
			...
			while (pNBLTemp) {
				Pass = CheckNetBufferList(pFilter, pNBLTemp, U_CNBL_RECEIVE);
